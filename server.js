...
var server = http.createServer(function(request, response) {
    ...
    if (path === '/index.html') { //路由就是模拟if else
        response.statusCode = 200
        response.setHeader('Content-Type', 'text/html;charset=utf-8')
        let string = fs.readFileSync('public/index.html').toString()
        const page1 = fs.readFileSync('db/page1.json').toString() //将文件内容转化为字符串格式
        const array = JSON.parse(page1) //需求：将字符串转换为数组中的li
        const result = array.map(item => `<li>${item.id}</li>`).join('')
        string = string.replace('{{page1}}', `<ul id="xxx">${result}</ul>`) //注意replace方法返回的是被修改后的数组，原数组仍存在
